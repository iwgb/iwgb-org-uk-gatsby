...
const FeaturedCampaign = ({
  ...
  <div
    className={`${styles.campaign} ${styles[direction]} row flex-nowrap my-5 `}
  >
    <div
    ...
    >
      ...
      <div className="flex-shrink-1 px-0">
        <div
          ...
          style={{ backgroundImage: `url(${image})` }}
        >
          <StripeText className={styles.stripe}>{topText}</StripeText>
          <br />
          ...
          <div className={`${styles.body} bg-white px-3 py-2`}>
            {body && <div>{body}</div>}
            <div className="mt-2">
              <Link to={ctaPath} className="mt-2">
                <Button>
                  <FormattedMessage id={ctaMessage} />
                </Button>
              </Link>
              <Link to={Paths.donate()} className="ms-2">
                <Button>
