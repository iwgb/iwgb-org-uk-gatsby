...
const Pagination = ({ page, pages, hasPrevious, hasNext, getPath }: Props) => (
  <div className="row justify-content-between align-items-center py-3">
    <div className={`w-auto ${!hasPrevious && 'hidden'}`}>
      <Link className="link-unstyled" to={getPath(page - 1)}>
        <IconButton icon={faArrowLeft} direction="left">
          Newer
        </IconButton>
      </Link>
    </div>
    ...
    <div className="w-auto">
      {hasNext && (
        <Link className="link-unstyled" to={getPath(page + 1)}>
          <IconButton icon={faArrowRight}>Older</IconButton>
        </Link>
      )}
    </div>
  </div>
